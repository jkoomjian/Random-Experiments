module Cms
  module MenuHelper
    ...
    def render_menu(options = {})
      ...
      # JK - Added 1/7/2014
      # set the login or logout state depending on if the user is logged in
      if current_user.guest?
        options[:items] << {:id=>"", :url=>"/login", :name=>"Login/Sign Up", :class => "right button"}
        # Add link for trial
        options[:items] << {:id=>"", :url=>"/trial", :name=>"Day Free Trial", :class => "right trial"}
      else
        # Add in members, account settings when logged in
        options[:items] << {:id=>"", :url=>"/members", :name=>"Member Home"}
        options[:items] << {:id=>"", :url=>"/members/settings", :name=>"Account Settings"}

        options[:items] << {:id=>"", :url=>"/cms/logout", :name=>"Logout", :class => "right button"}
      end
