...
module Cms
  class User < ActiveRecord::Base
    ...
    attr_accessible :login, :email, :name, :first_name, :last_name, :password, :password_confirmation, :expires_at, :trial_start, :subscription_status
    ...
    def display_name
      "#{first_name.capitalize} #{last_name.slice(0).capitalize}."
    end
    ...
    def in_group?(group_name)
      return !self.groups.index(Cms::Group.find_by_name(group_name)).nil?
    end

    def is_admin?
      return (self.in_group?("Cms Administrators") || self.in_group?("Content Editors"))
    end

    def to_s
      return "#{self.login}: #{self.first_name} #{self.last_name} [#{self.email}]"
    end
