module Cms
  class ContentType < ActiveRecord::Base
    ...
    def self.find_by_key(key)
      ...
      # Bug Fix: 1/8/14: vendor code matches and block with which is a superset of another content block name
      # ex. BlogComment will match a search for Comment
      # only match the class part of the path
      content_type = find(:first, :conditions => ["name like ? and (name like ? or name = ?)", "%#{class_name}", "%::#{class_name}", "#{class_name}"])
